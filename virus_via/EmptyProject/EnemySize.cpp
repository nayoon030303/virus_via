...
EnemySize::EnemySize(int px, int py)
{
    dead = false;
...
void EnemySize::Update()
{
    /*if (map[y * WIDTH + x] == MAP_PROPERTY_VISIT)
    {
        int a = 0;
    }
    if (map[y * WIDTH + x + ENEMY_WIDTH] == MAP_PROPERTY_VISIT)
    {
        int a = 0;
    }
    if (map[(y + ENEMY_HEIGHT) * WIDTH + x] == MAP_PROPERTY_VISIT)
    {
       int  a = 0;
    }
    if (map[(y + ENEMY_HEIGHT) * WIDTH + x + ENEMY_WIDTH] == MAP_PROPERTY_VISIT)
    {
        int a = 0;
    }*/
    if (map[y * WIDTH + x] == MAP_PROPERTY_VISIT ||
...
bool EnemySize::isDead()
{
    return dead;
}

D3DXVECTOR2 EnemySize::getPos()
{
